...
func main()  {
	...
	movies = append(movies, Movie{ID: "1", isbn: "4328277", Title: "Movie one", Director: &Director{Firstname: "rock", Lastname: "Doe"}} )
	movies = append(movies, Movie{ID: "2", isbn: "365946", Title: "movie 2", Director: &Director{Firstname: "jon", Lastname: "wick"}})
	r.HandlerFunc("/movies", getMovies).Methods("GET")
	r.HandlerFunc("/movies/{id}", getMovie).Methods("GET")
	r.HandlerFunc("/movies", createMoive).Methods("POST")
	r.HandlerFunc("/movies/{id}", updateMovie).Methods("PUT")
	r.HandlerFunc("/movies/{id}", deleteMovie).Methods("DELETE")


	fmt.Println("starting serve at port 8000")
